...
export default function Projects() {
  ...
  return (
    <section className="container" id="projects" style={{ paddingBottom: '20vh' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '0px' }}>
        {t.projects.items.filter((i: any) => i.featured !== false).map((item: any, idx: number) => {
          ...
          return (
            ...
            >
              ...
              {isVera ? (
                <div style={{ position: 'relative', width: '100%', height: isMobile ? 'auto' : '100%', display: 'flex', flexDirection: isMobile ? 'column' : 'row', padding: 'clamp(1.5rem, 5vw, 4rem)' }}>
                  {isMobile && (
                  ...
                  <div style={{ position: 'relative', zIndex: 20, width: isMobile ? '100%' : '45%', textAlign: 'left', pointerEvents: 'none' }}>
                    <div style={{ pointerEvents: 'auto' }}>
                      ...
                      <p style={{ fontSize: 'clamp(1.1rem, 4vw, 1.15rem)', color: 'var(--text)', opacity: 0.85, lineHeight: 1.6, fontFamily: 'var(--font-body)', fontWeight: 400, marginBottom: '0.5rem', marginTop: '1.5rem' }}>{item.description}</p>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--text)' }}>
                        <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.1em', borderBottom: '1px solid rgba(255,255,255,0.3)', paddingBottom: '2px' }}>{t.hero.projectMeta.viewCaseStudy}</span>
                ...
                <div style={{ padding: isMobile ? '0 clamp(1.5rem, 5vw, 4rem) clamp(1.5rem, 5vw, 4rem)' : 'clamp(1.5rem, 5vw, 4rem)', height: '100%', width: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                  <div style={{ position: 'relative', zIndex: 20 }}>
                    ...
                    <p style={{ maxWidth: '500px', color: 'var(--text)', fontFamily: 'var(--font-body)', fontWeight: 400, position: 'relative', zIndex: 20, marginTop: '1.5rem', lineHeight: 1.6, opacity: 0.85, fontSize: 'clamp(1.1rem, 4vw, 1.15rem)' }}>{item.description}</p>
                    <p style={{ position: 'relative', zIndex: 20, fontFamily: 'var(--font-mono)', fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: '1.5rem', letterSpacing: '0.1em', marginBottom: '1rem' }}>{item.tags}</p>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--text)' }}>
                        <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.1em', borderBottom: '1px solid rgba(255,255,255,0.3)', paddingBottom: '2px' }}>{t.hero.projectMeta.viewCaseStudy}</span>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="5" y1="12" x2="19" y2="12"></line><polyline points="12 5 19 12 12 19"></polyline></svg>
                    </div>
              ...
              {isVera && !isMobile && (
                <div style={{ position: 'absolute', right: 0, top: 0, width: '55%', height: '100%', overflow: 'hidden', zIndex: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '4vh 0' }}>
                  <div style={{ position: 'relative', zIndex: 2, cursor: 'default' }} onMouseEnter={(e) => { e.stopPropagation(); setCursorType('default'); }}>
                    <VeraShowcase />
                  </div>
                </div>
              )}
              ...
              {isIEEE && !isMobile && heroImage && (
                <div style={{ position: 'absolute', bottom: 0, left: 0, width: '100%', height: '42%', overflow: 'hidden', zIndex: 5, padding: '2rem', display: 'flex', justifyContent: 'center', alignItems: 'flex-end' }}>
                  <motion.img src={heroImage.src} whileHover={{ scale: 1.03 }} transition={{ duration: 0.8, ease: 'easeOut' }} style={{ width: '100%', height: '100%', objectFit: 'contain', objectPosition: 'bottom' }} />
                </div>
              )}
      ...
      <div style={{ display: 'flex', justifyContent: 'center', marginTop: '6rem' }}>
        <Link 
          ...
          style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', fontFamily: 'var(--font-mono)', fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.15em', color: 'var(--text)', padding: '1rem 2rem', border: '1px solid var(--border)', transition: 'all 0.3s ease', backgroundColor: 'rgba(255,255,255,0.02)' }}
          onMouseOver={(e) => { e.currentTarget.style.backgroundColor = 'var(--text)'; e.currentTarget.style.color = 'var(--bg)'; }}
          onMouseOut={(e) => { e.currentTarget.style.backgroundColor = 'rgba(255,255,255,0.02)'; e.currentTarget.style.color = 'var(--text)'; }}
        >
          {t.hero.archive} →
        </Link>
      </div>
